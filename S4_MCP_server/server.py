...
def write_file(filename: str, content: str) -> str:
    """Create or overwrite a file in the sandbox folder with content."""
    try:
        ensure_sandbox()
...
def read_file(filename: str) -> str:
    """Read the contents of a file in the sandbox folder."""
    try:
        filepath = get_safe_path(filename)
...
def append_file(filename: str, content: str) -> str:
    """Append content to a file in the sandbox folder."""
    try:
        ensure_sandbox()
        filepath = get_safe_path(filename)
...
def delete_local_file(filename: str) -> str:
    """Delete a file in the sandbox folder."""
    try:
        filepath = get_safe_path(filename)
        if not os.path.exists(filepath):
            return f"File {filepath} does not exist."
...
@mcp.tool()
def save_chat_history(filename: str = "default.txt") -> str:
    """Save the current auto-logged chat history to a specific filename."""
    try:
        import shutil
        ensure_sandbox()
        current_path = get_safe_path("current_chat.txt")
        new_path = get_safe_path(filename)
        if not os.path.exists(current_path):
            return "No chat history exists yet."
        shutil.copy(current_path, new_path)
        return f"Successfully saved chat history to {new_path}"
    except Exception as e:
